...
# --- O7/O8: two-stream attention LoRA overlap (decode-only) ----------------
# Run the LoRA shrink (sgemm_lora_a_fwd) on a side stream concurrent with the
# corresponding base attention GEMM (qkv_proj for O7, o_proj for O8) on the main
# stream; join before the LoRA expand atomic-adds the delta into base_output.
# Gated decode-only via the SGLANG_LORA_TWO_STREAM env var (same master switch
# as O1's MoE gate_up overlap). QKV and o_proj share one side stream — they run
# sequentially within an attention layer (qkv → attn → o_proj), so reusing a
# single global stream is fine.
_ATTN_LORA_SIDE_STREAM = None


def _get_attn_lora_side_stream():
    global _ATTN_LORA_SIDE_STREAM
    if _ATTN_LORA_SIDE_STREAM is None:
        _ATTN_LORA_SIDE_STREAM = torch.cuda.Stream()
    return _ATTN_LORA_SIDE_STREAM


def _attn_two_stream_active(x) -> bool:
...
class QKVParallelLinearWithLoRA(ColumnParallelLinearWithLoRA):
    ...
    def forward(self, input_: torch.Tensor):
        ...
        """
        if not self.set_lora or not _attn_two_stream_active(input_):
        ...
        bias = self.base_layer.bias if not self.base_layer.skip_bias_add else None
        side_stream = _get_attn_lora_side_stream()
...
class RowParallelLinearWithLoRA(BaseLayerWithLoRA):
    ...
    def forward(self, input_: torch.Tensor, skip_all_reduce=False, forward_batch=None):
        ...
        # O8: side-stream LoRA shrink concurrent with the base row-parallel GEMM
        # (e.g. o_proj). The shrink reads input_parallel — same input as the base
        # GEMM, no write conflict — so it can fork off, run on the side stream,
        # and rejoin before the all-reduce or expand consumes its result. Decode-
        # only, gated by SGLANG_LORA_TWO_STREAM (same master switch as O1/O7).
        two_stream = self.set_lora and _attn_two_stream_active(input_parallel)
        lora_a_output = None
        if two_stream:
            side_stream = _get_attn_lora_side_stream()
            side_stream.wait_stream(torch.cuda.current_stream())
