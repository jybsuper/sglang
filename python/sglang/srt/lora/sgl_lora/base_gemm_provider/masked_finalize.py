"""Shared-rank finalizer for the masked BF16 MoE row domain.

The materialized path stays on :meth:`MoeBaseProvider.finalize`.  This module
contains the retained shared-outer specialization: router weights are reduced
in rank space, then one from-scratch kernel combines weighted base rows with
the shared B tail and applies routed scaling to the complete sum.  Reductions
use a fixed order and every destination cell has exactly one writer.
...
def _validate_output_boundary(
    *,
    ...
    if (
        down_masked.ndim != 3
        or down_masked.shape[0] != num_local_experts
        or down_masked.shape[2] < 1
    ):
        raise ValueError("down_masked must be [num_local_experts, m_max, hidden]")
    hidden = down_masked.shape[2]
    ...
    if down_masked.dtype != torch.bfloat16:
        raise TypeError("masked BF16 finalizer requires BF16 base down rows")
    tensors = (
        down_masked,
        src2dst,
        routing.topk_ids,
