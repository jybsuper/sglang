"""Fused LoRA middle stages over the production masked MoE row domain.

This kernel is the production-shaped form of the retained Step-5 candidate. The
base GEMM writes ``[E, m_max, S * I]`` while LoRA uses canonical pair order,
so it explicitly crosses the domains through ``src2dst``:
...
The fused B paths accumulate the LoRA-B product and combine it with the BF16
base row in FP32 before activation.  They therefore implement the mathematical
``activation(base + A @ B)`` contract, but intentionally do not reproduce the
extra BF16 delta-materialization rounding of the serial reference.  The
activation exposed to down-A and the base down GEMM is rounded to the provider
BF16 ABI in both paths.
...
from sglang.srt.lora.sgl_lora.routing import ROUTE_ALIGNED, RouteView
...
def run_masked_fused_middle(
    ...
    *,
    ...
    """Run the production masked B-plus-activation family."""
    ...
    if bridge_gateup is None or b_gate_up is None:
        raise ValueError("b_activation requires bridge_gateup and b_gate_up")
