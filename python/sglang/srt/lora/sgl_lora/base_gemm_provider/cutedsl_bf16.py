...
Selected explicitly by the evidence-backed serving policy on SM90/SM100; the
...
class CuteDslBf16Provider(MaskedRowDomainProvider):
    ...
    def __init__(self, quant_info: SglLoraBf16QuantInfo):
        ...
        # compile the kept static scheduler (no packing ABI). A fixed-provider
        # server must reject the geometry rather than switching providers.
        if quant_info.num_local_experts > MAX_EXPERTS:
            raise ValueError(
                f"{quant_info.num_local_experts} local experts exceed the "
                f"fixed CuTeDSL provider's {MAX_EXPERTS}-expert packing"
            )
    ...
    def _compile_stage(
        ...
        """Compile and warm one stage at attach time."""
    ...
    def _token_width_for(self, m_max: int, expected_m: int) -> int:
        ...
        the fixed CuTeDSL provider rejects the workload.
        """
        ...
        raise ValueError(
            ...
            f"packing ({widest * self._max_token_clusters}) for the fixed "
            "CuTeDSL provider"
        )
