"""Selector-emitted BF16 LoRA-A kernels for the SGL MoE backend.

The serving policy retains three families:

* aligned grouped A, the general winner (implemented in ``bf16``);
* raw-route indexed A for the evidence-selected down-A decode path; and
* token-deduplicated shared-outer grouped A.

The caller owns route selection, launch-config selection, workspace lifetime,
and composition with B. Indexed A consumes a raw ``RouteView``; grouped A
consumes an aligned route.

Port provenance (the source launchers remain benchmark-only controls):

* ``grouped_lora_a`` wraps ``sgl_lora.bf16.grouped_lora_a``;
...
if TYPE_CHECKING:
    from sglang.srt.lora.sgl_lora.execution_plan import LoraASpec
...
def run_lora_a(
    ...
    *,
    ...
    if family in ("grouped", "token_dedup_grouped"):
        if output is None or routing is None or config is None:
            raise ValueError(f"{family} A requires output, routing, and config")
    ...
    if family == "indexed":
        if output is None or routing is None or config is None:
            raise ValueError("indexed A requires output, routing, and config")
        indexed_lora_a(
            input,
            weight,
            output,
