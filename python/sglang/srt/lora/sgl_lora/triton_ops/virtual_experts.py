"""Virtual-expert routing and LoRA A/B execution owned by ``sgl_lora``.

This is the environment-free execution core for the new LoRA engine.  Policy
choices are explicit arguments from the runner; legacy experimental flags and
temporary-package kernels are deliberately not imported here.
"""
    ...
    def _align_block_size(
        ...
        if num_experts < 1024:
            ...
            if num_pairs < num_experts + 1:
                max_num_tokens_padded = num_pairs * block_size
            else:
                max_num_tokens_padded = num_pairs + (num_experts + 1) * (block_size - 1)
