...
* **Graph capture** uses stable runner-workspace addresses, and every replay's
  K2 re-establishes the counts invariant. Direct callers retain a serial
  module-cache fallback. An eager exception BETWEEN K1 and K2 would leave
  scratch dirty; all allocation, lookup, and contract validation is therefore
  hoisted above the first launch.
...
def _expand_and_scatter_kernel(
    ...
    pair id. The key recompute remains local to the scatter half.
    """
    pid = tl.program_id(0)
