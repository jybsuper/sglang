...
def main():
    ...
    for m in range(bs):
        for k in range(TOP_K):
            ...
            inter[vt, RANK : 2 * RANK] = (hidden[m].float() @ uA[e].float().t()).to(
                dtype
            )
    ...
    def verdict(out, tag):
        ...
        for m in range(bs):
            for k in range(TOP_K):
                ...
                got = out[m, k].float()
                ge = max(ge, (got[:INTER] - gref).abs().max().item())
                gd = max(gd, gref.abs().max().item())
                up = max(up, (got[INTER:] - uref).abs().max().item())
                ud = max(ud, uref.abs().max().item())
        print(
            f"  [{tag}] GATE rel={ge/(gd+1e-9):.2%}  UP rel={up/(ud+1e-9):.2%}  "
