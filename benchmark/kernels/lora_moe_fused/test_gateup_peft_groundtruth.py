...
def main():
    ...
    ap.add_argument(
        "--adapter", default="/data/qwen35_35b_lora_alpha/adapter_model.safetensors"
    )
    ap.add_argument("--layer", type=int, default=0)
    ap.add_argument(
        "--experts", type=int, default=64, help="num owned experts to build"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=2.0,
        help="adapter scaling (alpha/r); applied to lora_b like prod",
    )
    args = ap.parse_args()
    ...
    gAf, uAf, gBf, uBf = (
        gA.float(),
        uA.float(),
        (gB * args.scaling).float(),
        (uB * args.scaling).float(),
    )
    xf = hidden.float()
    ...
    print(
        f"  GATE half: max_abs_err={gate_err:.3e}  ref_max={gate_den:.3e}  rel={gate_err/(gate_den+1e-9):.2%}"
    )
    print(
        f"  UP   half: max_abs_err={up_err:.3e}  ref_max={up_den:.3e}  rel={up_err/(up_den+1e-9):.2%}"
    )
    ...
    up_ok = up_rel < 0.05
    print(
        f"  GATE {'MATCH' if gate_ok else 'MISMATCH'} | UP {'MATCH' if up_ok else 'MISMATCH'} vs PEFT"
    )
    if gate_ok and not up_ok:
        print(
            "  => CONFIRMED BUG: gate half correct, up half wrong (up output uses gate_A shrink)."
        )
