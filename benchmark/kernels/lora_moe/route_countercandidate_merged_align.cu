// BENCHMARK-OWNED SNAPSHOT: provider-neutral counter-candidate; production must
...
warp_exclusive_scan(int v, unsigned mask = 0xffffffffu) {
  int original = v;
#pragma unroll
...
// the merged virtual expert id for flat slot `i` (range [-1,
// virtual_num_experts); -1 is the dropped/masked sentinel). The caller adds +1
// to get the histogram bucket (sentinel -> bucket 0), matching the native +1
// offset convention.
template <typename scalar_t>
__device__ __forceinline__ int
compute_virtual_id(const scalar_t *__restrict__ topk_ids,
                   const int32_t *__restrict__ token_lora_mapping, size_t i,
                   int top_k, int num_experts_for_weight,
                   int local_expert_offset, int local_num_experts,
                   bool ep_local, bool shared_outer, bool compact) {
  ...
  if (ep_local) {
    bool owned = base >= local_expert_offset &&
  ...
  if (compact)
    return base - local_expert_offset;
...
__global__ void count_and_sort_expert_tokens_kernel(
    ...
    size_t numel, int top_k, int num_experts_for_weight,
    int local_expert_offset, int local_num_experts, bool ep_local,
    bool shared_outer, bool do_skip, bool compact) {
...
__global__ void moe_align_block_size_kernel(
    const scalar_t *__restrict__ topk_ids,
    const int32_t *__restrict__ token_lora_mapping,
    bool *__restrict__ token_lora_mask, int32_t *__restrict__ sorted_token_ids,
    ...
    int32_t max_num_tokens_padded, int top_k, int num_experts_for_weight,
    int local_expert_offset, int local_num_experts, bool ep_local,
    bool shared_outer, bool do_skip, bool compact) {
...
// Single-block fused variant: does fill + histogram + scan + expert_ids +
// scatter in ONE threadblock (one launch), eliminating the separate
// count_and_sort kernel AND its redundant re-computation of the virtual id
// (cached in shared `svids`). Only valid for small numel (the scatter is
// single-block); the wrapper routes large numel (prefill) to the 2-kernel path.
// do_skip is implied (this is the decode hot path); dropped slots are simply
// never scattered.
template <typename scalar_t>
__global__ void fused_align_scatter_kernel(
    ...
    int32_t *__restrict__ total_tokens_post_pad, int32_t num_experts,
    int32_t block_size, size_t numel, int32_t *__restrict__ cumsum,
    const int32_t scan_size, int32_t max_num_tokens_padded, int top_k,
    int num_experts_for_weight, int local_expert_offset, int local_num_experts,
    bool ep_local, bool shared_outer, bool do_skip, bool compact) {
  extern __shared__ int32_t smem[];
  int32_t *shared_counts = smem;                 // [num_experts]
  int32_t *prefix = shared_counts + num_experts; // [num_experts + 1]
  int32_t *scan_buf = prefix + num_experts + 1;  // [scan_size]
...
template <typename scalar_t> struct MoeLoraMergedAlignKernel {
  static void
  run(tvm::ffi::TensorView topk_ids, tvm::ffi::TensorView token_lora_mapping,
      tvm::ffi::TensorView token_lora_mask, int64_t num_experts,
      int64_t block_size, tvm::ffi::TensorView sorted_token_ids,
      tvm::ffi::TensorView expert_ids, tvm::ffi::TensorView num_tokens_post_pad,
      tvm::ffi::TensorView cumsum_buffer, bool pad_sorted_token_ids,
      int64_t top_k, int64_t num_experts_for_weight,
      int64_t local_expert_offset, int64_t local_num_experts, bool ep_local,
      bool shared_outer, bool do_skip, bool compact, bool fuse_scatter) {
    ...
    // (typically 385). Compact: local_num_experts+1 (typically 49). Both use
    // the same single-block align path (valid for any bucket count <= 1024 that
    // fits shared memory); the v2 (>1024) regime keeps the old path via the
    // wrapper.
    RuntimeCheck(num_experts <= 1024,
                 "moe_lora_merged_align: num_experts (bucket count) must be <= "
                 "1024, got ",
                 num_experts);
    // compact buckets hold LOCAL ids and restore global expert ids as
    // (left-2+offset). For the sentinel bucket 0 that yields (offset-1), NOT
    // the -1 the GEMM expects to skip -- only safe when do_skip empties bucket
    // 0.
    RuntimeCheck(!compact || do_skip,
                 "moe_lora_merged_align: compact requires do_skip (sentinel "
                 "bucket must be empty)");

    const scalar_t *topk_ids_ptr =
        static_cast<const scalar_t *>(topk_ids.data_ptr());
    const int32_t *tlm_ptr =
        static_cast<const int32_t *>(token_lora_mapping.data_ptr());
    bool *token_lora_mask_ptr = static_cast<bool *>(token_lora_mask.data_ptr());
    int32_t *sorted_token_ids_ptr =
        static_cast<int32_t *>(sorted_token_ids.data_ptr());
    int32_t *expert_ids_ptr = static_cast<int32_t *>(expert_ids.data_ptr());
    int32_t *num_tokens_post_pad_ptr =
    ...
    if (fuse_scatter) {
      ...
      // shared for the scatter cursor [num_experts] and cached virtual ids
      // [numel].
      const size_t shmem = (num_experts + (num_experts + 1) + scan_size +
                            WARP_SIZE + num_experts + numel) *
                           sizeof(int32_t);
      auto fused = moe_lora_merged::fused_align_scatter_kernel<scalar_t>;
      LaunchKernel(dim3(1), dim3(threads), stream, shmem)(
          fused, topk_ids_ptr, tlm_ptr, token_lora_mask_ptr,
          sorted_token_ids_ptr, expert_ids_ptr, num_tokens_post_pad_ptr,
          (int32_t)num_experts, (int32_t)block_size, numel, cumsum_buffer_ptr,
          (int32_t)scan_size, (int32_t)max_num_tokens_padded, (int)top_k,
          (int)num_experts_for_weight, (int)local_expert_offset,
          (int)local_num_experts, ep_local, shared_outer, do_skip, compact);
      return;
    }
    ...
    LaunchKernel(dim3(2), dim3(threads), stream, shared_mem_size)(
        align_kernel, topk_ids_ptr, tlm_ptr, token_lora_mask_ptr,
        sorted_token_ids_ptr, expert_ids_ptr, num_tokens_post_pad_ptr,
        (int32_t)num_experts, (int32_t)block_size, numel, cumsum_buffer_ptr,
        pad_sorted_token_ids, (int32_t)scan_size,
        (int32_t)max_num_tokens_padded, (int)top_k, (int)num_experts_for_weight,
        (int)local_expert_offset, (int)local_num_experts, ep_local,
        shared_outer, do_skip, compact);
    ...
    auto sort_kernel =
        moe_lora_merged::count_and_sort_expert_tokens_kernel<scalar_t>;
    LaunchKernel(dim3(actual_blocks), dim3(block_threads), stream)(
        sort_kernel, topk_ids_ptr, tlm_ptr, sorted_token_ids_ptr,
        cumsum_buffer_ptr, numel, (int)top_k, (int)num_experts_for_weight,
        (int)local_expert_offset, (int)local_num_experts, ep_local,
        shared_outer, do_skip, compact);
  }
};
