...
def _generate(
    ...
    *,
    ...
    body, elapsed_ms = _request_json("POST", f"{base_url}/generate", payload=payload)
    return {
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(aliases) < 3:
        raise ValueError(
            "at least three --alias values are required to force slot reuse"
        )
    ...
    for alias in aliases:
        events.append(
            {
                "operation": "load",
                **_load_alias(args.base_url, alias, args.adapter_path),
            }
        )
    ...
    _assert_equal(
        "mixed replay after slot churn", mixed_ids, mixed_reference["output_ids"]
    )
    ...
    events.append(
        {
            "operation": "reload",
            **_load_alias(args.base_url, aliases[0], args.adapter_path),
        }
    )
    reloaded = _generate(
        args.base_url,
    ...
    _assert_equal(
        "unload/reload", reloaded["output_ids"], adapter_reference["output_ids"]
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--alias", nargs="+", default=["lora_a", "lora_b", "lora_c"])
