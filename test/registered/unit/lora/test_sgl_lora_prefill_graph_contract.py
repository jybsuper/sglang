"""Hermetic contracts for SGL MoE-LoRA prefill CUDA-graph metadata.
...
def test_sgl_lora_is_the_only_moe_engine_eligible_for_prefill_graph() -> None:
    ...
    manager.prefill_cuda_graph_backend = "full"

    manager.lora_execution_engine = "sgl_lora"
    assert manager.supports_prefill_cuda_graph

    manager.lora_execution_engine = "legacy"
    ...
    manager.prefill_cuda_graph_backend = "tc_piecewise"
    assert not manager.supports_prefill_cuda_graph
...
def test_graph_metadata_selects_prefill_buffer_by_batch_identity() -> None:
    ...
    extend_mode = SimpleNamespace(is_extend=lambda: True)
